...
@app.get("/api/health")
def health(): return {"ok": True, "model": "Qwen/Qwen-Image-2.1"}
...
async def generate(prompt: str=Form(...), mode: str=Form("generate"), width:int=Form(1024), height:int=Form(1024), steps:int=Form(30), seed:int=Form(-1), transparent:bool=Form(False), images:List[UploadFile]=File(default=[])):
    ...
    if mode=="edit" and not refs:
        return {"error":"图片编辑模式至少需要 1 张参考图"}
    image,used_seed=engine.generate(prompt,width,height,steps,seed,refs)
    name=f"{uuid.uuid4().hex}.png"; path=os.path.join(OUTPUT,name); image.save(path)
    return {"url":f"/api/output/{name}","seed":used_seed,"mode":mode,"references":len(refs)}
